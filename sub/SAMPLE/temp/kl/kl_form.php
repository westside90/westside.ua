...
function ost_start($date)
	{
	$sql1 = mysql_query ("SELECT SUM(`summa`) FROM `kassa` WHERE `document` = 1 AND `status` = 1 AND `type` = 0 AND DATE(`date`) <= '".$date."'");
	$sql2 = mysql_query ("SELECT SUM(`summa`) FROM `kassa` WHERE `document` = 1 AND `status` = 1 AND `type` = 1 AND DATE(`date`) <= '".$date."'");
