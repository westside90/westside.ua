...
<?php
$max_date = getvariable ('max_date', mysql_result (mysql_query ("SELECT max(`date`) FROM `plan` WHERE `document` = 4"), 0));
$min_date = getvariable ('min_date', mysql_result (mysql_query ("SELECT min(`date`) FROM `plan` WHERE `document` = 4"), 0));	
?>
<!-- Отримуємо діапазон дат по замовчуванню -->

<!-- Форма з вибором діапазону дат -->
<form ACTION = 'bank.php?page=5' METHOD = 'POST'>
	<p align = 'center'>
...
$sql = mysql_query ("SELECT DISTINCT `date` FROM `bv` WHERE `status` = 1 AND DATE(date) BETWEEN '$min_date' AND '$max_date' ORDER BY `date`");
...
for ($i = 0; $i < $k; $i++)
	{
	$sql = mysql_query ("SELECT `type`,`date`,`make_type`,`make`,`summa`,`description` FROM `bv` WHERE `status` = 1 AND DATE(date) = '".$zvit[$i]['date']."'");
...
//Дані для звіту
//Початковий залишок
$sql1 = mysql_query ("SELECT SUM(summa) FROM `bv` WHERE `status` = 1 AND `type` = 0 AND DATE(date) < '$min_date'");
$sql2 = mysql_query ("SELECT SUM(summa) FROM `bv` WHERE `status` = 1 AND `type` = 1 AND DATE(date) < '$min_date'");
...
for ($i = 0; $i < $k; $i++)
	{
	$sql1 = mysql_query ("SELECT SUM(summa) FROM `bv` WHERE `status` = 1 AND `type` = 0 AND DATE(date) = '".$zvit[$i]['date']."'");
	$sql2 = mysql_query ("SELECT SUM(summa) FROM `bv` WHERE `status` = 1 AND `type` = 1 AND DATE(date) = '".$zvit[$i]['date']."'");
