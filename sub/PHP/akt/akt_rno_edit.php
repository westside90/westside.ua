...
if (isset($_GET['id']))
	{
	$_GET['old'] = $_GET['id'];
	$sql = mysql_query ("SELECT `number`,`date`,`kontragent`,`rahunok`,`prymitka`,`posluga`,`kkst`,`cost` FROM `akt` 
...
function poslugi_list($selected, $num, $k)
	...
	echo "<select name = 'posluga[$num]' onchange = \"document.forms.form.action = '?k=".$k."&old=".$_GET['old']."'; document.form.submit();\">";
	echo "<option value = '0'";
...
$form['save'] = "<input type = 'button'
				...
				onClick = \"if (confirm('Зберегти документ?'))
						{
						document.forms.form.action='akt_rno_update.php?k=".$k."&old=".$_GET['old']."';
