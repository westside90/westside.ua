...
function poslugi_list($selected, $num, $k)
	...
	echo "<select name = 'posluga[$num]' onchange = \"document.forms.form.action = '?k=".$k."&document=".$_GET['document']."'; document.form.submit();\">";
	echo "<option value = '0'";
...
$form['save'] = "<input type = 'button'
				...
				onClick = \"if (confirm('Зберегти документ?'))
						...
						if (confirm('Провести документ?'))
							{ document.forms.form.action='akt_add.php?k=".$k."&status=1&document=".$_GET['document']."'; }
						else
							{ document.forms.form.action='akt_add.php?k=".$k."&status=0&document=".$_GET['document']."'; }
...
$form['add_line'] = "<input type = 'button'
					...
					onclick = \"document.forms.form.action='?k=".($k + 1)."&document=".$_GET['document']."'; document.form.submit(); window.outerHeight=(window.outerHeight+30);\">";
$form['onblur'] = "onBlur = \"document.forms.form.action = '?k=".$k."&document=".$_GET['document']."'; document.form.submit();\"";
//Кнопки
