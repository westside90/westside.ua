...
if (isset($_GET['id']))
	{
	$_GET['old'] = $_GET['id'];
	$sql = mysql_query ("SELECT `number`,`date`,`kontragent`,`prymitka`,`posluga`,`kkst`,`cost` FROM `akt` 
						WHERE `document` = ".$_GET['document']." 
						AND `number` = (SELECT `number` FROM `akt` WHERE `id` = ".$_GET['old'].")");
	$k = mysql_num_rows ($sql);
	while ($akt[] = mysql_fetch_array ($sql, MYSQL_ASSOC));
	unset($akt[$k]);

	//Перетворюємо масив
	$akt['number'] = $akt[0]['number'];
	$akt['date'] = $akt[0]['date'];
	$akt['kontragent'] = $akt[0]['kontragent'];
	$akt['prymitka'] = $akt[0]['prymitka'];
	for ($i = 0; $i < $k; $i++)
		{
		$akt['posluga'][$i] = $akt[$i]['posluga'];
		$akt['kkst'][$i] = $akt[$i]['kkst'];
		$akt['cost'][$i] = $akt[$i]['cost'];
		unset($akt[$i]);
		}
	//Перетворюємо масив
...
function poslugi_list($selected, $num, $k)
	...
	echo "<select name = 'posluga[$num]' onchange = \"document.forms.form.action = '?k=".$k."&document=".$_GET['document']."&old=".$_GET['old']."'; document.form.submit();\">";
	echo "<option value = '0'";
...
function kontragent_list($selected, $k, $document)
	...
	unset($kontragent[$n]);
	echo "<select name = 'kontragent' onchange =\"document.forms.form.action = '?k=".$k."&document=".$_GET['document']."&old=".$_GET['old']."'; document.form.submit();\">";
	echo "<option value = '0'";
	if ($selected == 0) {echo " selected";}
	echo "> ...вибрати "; 
...
$form['save'] = "<input type = 'button'
				...
				onClick = \"if (confirm('Зберегти документ?'))
						...
						if (confirm('Провести документ?'))
							{ document.forms.form.action='akt_update.php?k=".$k."&status=1&document=".$_GET['document']."&old=".$_GET['old']."'; }
						else
							{ document.forms.form.action='akt_update.php?k=".$k."&status=0&document=".$_GET['document']."&old=".$_GET['old']."'; }
...
$form['add_line'] = "<input type = 'button'
					...
					onclick = \"document.forms.form.action='?k=".($k + 1)."&document=".$_GET['document']."&old=".$_GET['old']."'; document.form.submit(); window.outerHeight=(window.outerHeight+30);\">";
$form['onblur'] = "onBlur = \"document.forms.form.action = '?k=".$k."&document=".$_GET['document']."&old=".$_GET['old']."'; document.form.submit();\"";
//Кнопки

//Підключаємо шаблон форми
require_once "../../HTML/akt/akt_form.html";
//Підключаємо шаблон форми
