...
if (isset($_GET['id']))
	{
	$_GET['old'] = $_GET['id'];
	$sql = mysql_query ("SELECT `number`,`date`,`kontragent`,`rahunok`,`prymitka`,`tovar`,`kkst`,`cost` FROM `nakladna` 
						WHERE `document` = 2 
	...
	$nakladna['rahunok'] = $nakladna[0]['rahunok'];
	for ($i = 0; $i < $k; $i++)
		{
...
$form['save'] = "<input type = 'button'
				...
				onClick = \"if (confirm('Зберегти документ?'))
						{
						document.forms.form.action='nakladna_rno_update.php?old=".$_GET['old']."';
