...
<?php
$max_date = getvariable ('max_date', mysql_result (mysql_query ("SELECT max(`date`) FROM `nakladna` WHERE `status` = 1"), 0));
$min_date = getvariable ('min_date', mysql_result (mysql_query ("SELECT min(`date`) FROM `nakladna` WHERE `status` = 1"), 0));	
?>
<!-- Отримуємо діапазон дат по замовчуванню -->

<!-- Форма з вибором діапазону дат -->
<form ACTION = 'nakladna.php?page=9' METHOD = 'POST'>
	<p align = 'center'>
		<input document = 'date'  name = 'min_date' value = '<?= $min_date ?>' id = 'text'>
		<input document = 'submit' value = 'Показати' id = 'button'>
		<input document = 'date'  name = 'max_date' value = '<?= $max_date ?>' id = 'text'>
		</p>
	</form>
<!-- Форма з вибором діапазону дат -->
...
for ($i = 0; $i < $k; $i++)
	{
	$sql = mysql_query ("SELECT DISTINCT `number`,`date` FROM `nakladna` WHERE `tovar` = ".$zvit[$i]['id']." AND `document` = 0 AND `status` = 1");
...
for ($i = 0; $i < $k; $i++)
	...
	for ($j = 0; $j < $zvit[$i]['nakladna']['k']; $j++)
		{
		$sql = "SELECT `number` FROM `nakladna` WHERE `document` = 1 AND `tovar` = ".$zvit[$i]['id']." AND `partia` = ".$zvit[$i]['nakladna'][$j]['number']." AND `status` = 1";
