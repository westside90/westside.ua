...
function kontragent_list($selected, $k)
	...
	unset($kontragent[$n]);
	echo "<select name = 'kontragent' onchange =\"document.forms.form.action = '?page=2&k=".$k."'; document.form.submit();\">";
	echo "<option value = '0'";
...
function tovary_list($selected, $num, $k)
	...
	echo "<select name = 'tovar[$num]' onchange = \"document.forms.form.action = '?page=2&k=".$k."'; document.form.submit();\">";
	echo "<option value = '0'";
	if ($selected == 0) {echo " selected";}
...
function partia_list($selected, $selected_tovar, $date, $num, $k)
	...
	if ($selected_tovar == 0)
		...
		echo "<select name = 'partia[$num]' onchange = \"document.forms.form.action = '?page=2&k=".$k."'; document.form.submit();\">";
		echo "<option value = '0'";
		if ($selected == 0) {echo " selected";}
...
$form['save'] = "<input type = 'button'
				...
				onClick = \"if (confirm('Зберегти документ?'))
						...
						if (confirm('Провести документ?'))
							{ document.forms.form.action='nakladna_rashod_add.php?k=".$k."&status=1'; }
						else
							{ document.forms.form.action='nakladna_rashod_add.php?k=".$k."&status=0'; }
