...
if (isset($_GET['id']))
	{
	$_GET['old'] = $_GET['id'];
	$sql = mysql_query ("SELECT `prymitka`,`tovar`,`kkst`,`cost` FROM `nakladna` 
						WHERE `document` = 3 
...
$form['save'] = "<input type = 'button'
				...
				onClick = \"if (confirm('Зберегти документ?'))
						...
						if (confirm('Провести документ?'))
							{ document.forms.form.action='nakladna_ost_update.php?k=".$k."&status=1'; }
						else
							{ document.forms.form.action='nakladna_ost_update.php?k=".$k."&status=0'; }
...
require_once "../../HTML/nakladna/nakladna_ost_form.html";
//Підключаємо шаблон форми
